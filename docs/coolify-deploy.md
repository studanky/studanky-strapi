...
Alternative production deployment using the Coolify-native model:
...
The standalone [Docker Compose deployment](./deployment.md) remains a separate
option. Do not deploy the root `docker-compose.yml` in Coolify; it contains a
...
Backups are local in this setup. The operational requirement is that **Kopia
backs up the Coolify host data,
...
- Required Strapi secret values (`APP_KEYS` uses two independent values)
...
6. Storage: local Coolify/server storage for the layout documented here.
...
Configure the volume so Admin Panel uploads survive redeploys.
...
Strapi's newsletter limiter is process-local and keyed by an HMAC hash of the
normalized email. Configure request-size and rate limits on the trusted Coolify
proxy or edge layer as the primary ingress control.

Configure these optional SMTP variables when email sending is enabled:
...
This Coolify layout uses local backup storage. For independent host-loss
protection, configure S3-compatible backup storage in Coolify; uploads can also
use the S3-compatible provider already configured in this repository.
