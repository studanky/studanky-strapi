...
| `canonical-spring-name-migration.test.ts` | 1.5.0 Knex migration                                                                                                  | SQLite draft/published fixtures, source-locale selection, canonical copying, normalization, invariant preservation, inconsistent/missing-source rollback, fresh-DB no-op          |
...
| `locale.test.ts`                          | shared locale helpers                                                                                                 | Flutter tag canonicalization, configured-index reuse, script/base/sibling/default ordering, unsupported locales and deduplication                                                 |
...
| `spring-chmu-sync.test.ts`                | `syncFromChmu` localization behavior                                                                                  | Czech-only create/publish, canonical scalar allowlist propagation, no translated-description/status/source write, missing-Czech/config/documentId errors, compatible stats        |
...
| `spring-preview.test.ts`                  | preview service                                                                                                       | exact/base/sibling/default/source fallback, invalid-source degradation, served locale and teaser boundary                                                                         |
| `spring-read-localization.test.ts`        | map/search/full-detail services                                                                                       | per-document row selection, bounded aggregate invalid-source logging/degradation, visible global-config failures, source fallback, distance/limit behavior and query preservation |
| `spring-source-locale-lifecycle.test.ts`  | source metadata lifecycle                                                                                             | configured locale derivation, ČHMÚ/manual assignment, cross-row conflict rejection, null-sync tolerance, explicit backfill and 400 immutability validation                        |
